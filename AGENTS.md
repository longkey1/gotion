...
- `cmd/` — Cobra commands
  - `root.go` — root command. `PersistentPreRunE` first rejects write commands when read-only mode is enabled (`isWriteCommand` walks the command's parent chain checking the `write` annotation set by `create.go`/`update.go`), then refreshes an expired MCP token before any command except `auth`/`config`/`version`/`help`/`completion` (`skipTokenRefresh` walks the command's parent chain); refresh takes an exclusive file lock (`config.LockToken`) and re-reads the token file under the lock, so a token another process already refreshed is never refreshed twice; an `invalid_grant` refresh failure explains Notion's refresh-token expiry (180 days after initial authorization, or 30 days unused)
  ...
  - `create.go` — `gotion create` builds a page from stdin or `--file` via `gotion.ParseInput`; `--title` overrides the input's title property, `--parent` + `--parent-type page_id|database_id|data_source_id` set the parent (parent ID also goes through `ExtractPageID`). MCP backend only — the API client returns an "unsupported" error. Annotated with `write: true` so `root.go` blocks it in read-only mode
  - `update.go` — `gotion update <page_id>` updates properties and/or content from the same input formats; `--properties-only` / `--content-only` are mutually exclusive. MCP backend only. Annotated with `write: true` so `root.go` blocks it in read-only mode
  ...
  - `config.go` — `gotion config` prints the effective configuration, including the read-only flag, with masked secrets (`maskToken`) and which sources (env vars, config file, token file) are present
...
- `internal/gotion/config/` — configuration and token storage. `Load` uses Viper with priority env vars (`GOTION_BACKEND`, `GOTION_API_TOKEN`, `GOTION_API_CLIENT_ID`, `GOTION_API_CLIENT_SECRET`, `GOTION_READ_ONLY`) > `~/.config/gotion/config.toml` (including `read_only`) > `NOTION_TOKEN` env fallback > token file. `TokenData` (JSON, written 0600) tracks backend, access/refresh tokens, and expiry; `SaveToken` writes atomically (temp file + rename) so readers never see a partial file; `IsTokenExpired` uses a 5-minute margin and `NeedsRefresh` additionally requires a refresh token. `lock.go` provides `LockToken`, a flock-based exclusive lock on `token.json.lock` used to serialize token refresh across processes. `Validate` requires a token; `ValidateOAuth` requires client_id/client_secret
...
- `internal/gotion/config/lock_test.go` — `LockToken` acquire/release, contention timeout, waiting for a holder to release; `SaveToken` leaves no temp files
...
- `internal/notion/mcp/oauth_test.go` — PKCE generation (RFC 7636 relationship), endpoint discovery, dynamic client registration, code exchange against an httptest metadata server, and token endpoint error parsing (`OAuthError`)
...
- Read-only mode: `read_only = true` (config file) or `GOTION_READ_ONLY=true` makes `create`/`update` fail immediately in `PersistentPreRunE`, before any Notion API call — useful when exposing the CLI to an LLM or other automation that should never write
