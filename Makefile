...
build: ## Build the binary to ./bin/
	@mkdir -p bin
	go build -ldflags "\
		-X github.com/longkey1/gotion/internal/version.Version=$$(git describe --tags --always --dirty 2>/dev/null || echo dev) \
		-X github.com/longkey1/gotion/internal/version.CommitSHA=$$(git rev-parse --short HEAD 2>/dev/null || echo unknown) \
		-X github.com/longkey1/gotion/internal/version.BuildTime=$$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
		-o bin/$(PRODUCT_NAME)
